$(document).ready(function(){
    ...
    $(document).on('click','.delete' ,function() {
     ...
     // Intercept form submission
     $('#applySort').on('click', function(event) {
        event.preventDefault(); // Prevent default form submission behavior
        // Get the entered ID number
        let sortBy = $('#sortBy').val();
        let sortStatus = $('#status_sort').val();
        $('#studentIDBox').val('')
        console.log(sortBy)
        console.log(sortStatus)
        $.post({
            url: '/sort_exit_interview_admin_view/',
            data: { 'sortBy': sortBy , 'sortStatus': sortStatus },
            beforeSend: function(xhr, settings) {
                xhr.setRequestHeader("X-CSRFToken", csrftoken);
            },
            headers: {'X-CSRFToken': csrftoken}, 
            success: function(response) {
                ...
                let infoTable = tempDiv.querySelector('#info_table');
                
                // Log the infoTable or manipulate it as needed
                console.log(infoTable.innerHTML)
                if (infoTable) {
                    $('#info_table').html(infoTable.innerHTML)
                } else {
